...
id=0;model="a-IGNN";d=chameleon;gpu=$id;log_path=logs/$model;mkdir -p $log_path;nohup python -u -m scripts.03aignn_search --gpu=$gpu --dataset=$d --n_trials=256 --n_jobs=3 > $log_path/$id.log 2>&1 & echo $!

id=0;model="a-IGNN";d=products;gpu=$id;log_path=logs/$model;mkdir -p $log_path;nohup python -u -m scripts.03aignn_search --gpu=$gpu --dataset=$d --n_trials=256 --n_jobs=2 --repeat=1 > $log_path/$id.log 2>&1 & echo $!

# search on public splits
id=0;model="a-IGNN";d=chameleon;gpu=$id;log_path=logs/$model/public;mkdir -p $log_path;nohup python -u -m scripts.03aignn_search --gpu=$gpu --dataset=$d --n_trials=256 --n_jobs=3 --public True > $log_path/$id.log 2>&1 & echo $!
```
"""
