...
import torch
import tqdm
from helpers.classes import ActionNetArgs, ActivationType, EnvArgs, GumbelArgs
...
class Experiment(object):
    def __init__(self, args: Namespace):
        ...
        set_seed(args.seed)
        self.device = set_device(args.gpu)
    ...
    def run(self) -> Tuple[Tensor, Tensor]:
        dataset = self.dataset.load(source=self.source, seed=self.seed, pos_enc=self.pos_enc)
        ...
        env_args = \
            EnvArgs(model_type=self.env_model_type, num_layers=self.env_num_layers, env_dim=self.env_dim,
                    ...
                    act_type=env_act_type, metric_type=self.metric_type, in_dim=dataset.x.shape[1], out_dim=out_dim,
        ...
        for num_fold in folds:
            ...
            best_losses_n_metrics, edge_ratios, time_ =\
        ...
        if len(folds) > 1:
            ...
            print(
                ...
                f'Final Test: {round(metrics_mean[2], self.decimal)} ± {round(metrics_std[2], self.decimal)}'
            )

        save_to_csv_files(
            results={"acc": f"{metrics_mean[2]*100:.2f} ± {metrics_std[2]*100:.2f}"},
            insert_info={
                "dataset": self.dataset.name,
                "model": "CoGNN",
            },
            append_info={
                "time": f"{np.array(time_list).mean():.2f}",
                "args": env_args,
                "source": self.source,
            },
        ...
        return metrics_mean, edge_ratios, np.array(time_list).mean()

    def single_fold(
        self, dataset_by_split: DatasetBySplit, gumbel_args: GumbelArgs, env_args: EnvArgs,
        action_args: ActionNetArgs, num_fold: int
    ) -> Tuple[LossesAndMetrics, OptTensor]:
        model = CoGNN(
            gumbel_args=gumbel_args, env_args=env_args, action_args=action_args, pool=self.pool
        ...
        return best_losses_n_metrics, edge_ratios, time_
    ...
    def train_and_test(self, dataset_by_split: DatasetBySplit, model, optimizer, scheduler, pbar, num_fold: int)\
        ...
        return best_losses_n_metrics, edge_ratios, (time.time() - t_s) / (epoch + 1) * 100
    ...
    def test(self, loader, model, split_mask_name: str, calc_edge_ratio: bool)\
        ...
        total_y = None
        for data in loader:
            # print(self.batch_norm and (data.x.shape[0] == 1 or data.num_graphs == 1))
