...
from helpers.classes import ActivationType, ModelType, Pool
...
from helpers.dataset_classes.root_neighbours_dataset import \
    RootNeighboursDataset
...
class DataSet(Enum):
    ...
    def get_family(self) -> DataSetFamily:
        if self in [DataSet.roman_empire, DataSet.amazon_ratings, DataSet.minesweeper,DataSet.tolokers, DataSet.questions, DataSet.chameleon,DataSet.squirrel,DataSet.flickr,DataSet.blogcatalog,DataSet.actor]:
        ...
        elif self in [DataSet.cora, DataSet.pubmed,DataSet.photo,DataSet.wikics]:
    ...
    def load(self, source:str, seed: int, pos_enc: PosEncoder) -> List[Data]:
        ...
        if self.get_family() is DataSetFamily.heterophilic:
            ...
            dataset = load_data(
                dataset_name=self.name.replace('_','-'),
    ...
    def get_metric_type(self) -> MetricType:
        if self.get_family() in [DataSetFamily.social_networks, DataSetFamily.proteins, DataSetFamily.homophilic]\
                or self in [DataSet.roman_empire, DataSet.amazon_ratings,DataSet.flickr,DataSet.blogcatalog,DataSet.actor,DataSet.squirrel,DataSet.chameleon, DataSet.cycles]:
