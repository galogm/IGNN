...
class ACMGCN(nn.Module):
    def __init__(
        ...
        if args.acm_model == "acmgcnpp":
            self.mlpX = MLP_acm(
                in_features, args.nhid, args.nhid, num_layers=args.init_layers_X, dropout=0
            )
        ...
        self.fea_param, self.xX_param = Parameter(torch.FloatTensor(1, 1).to(device)), Parameter(
    ...
    def forward(self, x, return_Z=False):
        ...
        if (
            ...
            if self.acm_model == "acmgcnpp":
                fea2 = self.gcns[1](
                    fea1 + xX, self.adj_low, self.adj_high, self.adj_low_unnormalized
                )
            else:
...
class GraphConvolution(nn.Module):
    def __init__(
        ...
        self.att_struc_low = Parameter(torch.FloatTensor(1 * out_features, 1))
    ...
    def forward(self, input, adj_low, adj_high, adj_low_unnormalized):
        ...
        if self.acm_model == "mlp":
            ...
            return 3 * (
                self.att_low * output_low + self.att_high * output_high + self.att_mlp * output_mlp
            )
        else:
            if self.variant:
