...
for idx in range(run):
    ...
    if graph_dgl.name == "arxiv_ogb":
        ...
        #     test_mask.nonzero().flatten(),
        # )
        if graph_dgl.name in ['roman-empire_critical','chameleon_critical']:
            train_mask = graph_dgl.ndata['train_masks'][:,idx]
            val_mask = graph_dgl.ndata['val_masks'][:,idx]
            test_mask = graph_dgl.ndata['test_masks'][:,idx]
            train_idx, val_idx, test_idx = (
                train_mask.nonzero().flatten(),
                val_mask.nonzero().flatten(),
                test_mask.nonzero().flatten(),
