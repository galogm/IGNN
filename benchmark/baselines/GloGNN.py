...
class GloGNN(nn.Module):
    def __init__(
        ...
        # ---------------- Model -------------------
        self.orders_weight = Parameter(torch.ones(self.orders, 1) / self.orders, requires_grad=True)
        ...
        self.diag_weight = Parameter(torch.ones(self.nclass, 1) / self.nclass, requires_grad=True)
    ...
    def norm_func2(self, x, h0, adj):
        ...
        z = torch.mm(
            coe1 * xx + self.beta * a_sum - self.gamma * coe1 * hx,
    ...
    def order_func3(self, x, res, adj):
        ...
        orders_para = torch.mm(
            torch.relu(torch.mm(x, self.orders_weight_matrix)), self.orders_weight_matrix2
        )
