...
def relu_kernel_transformation(data, is_query, projection_matrix=None, numerical_stabilizer=0.001):
    ...
    if projection_matrix is None:
        ...
        ratio = 1.0 / torch.sqrt(torch.tensor(projection_matrix.shape[0], torch.float32))
...
def softmax_kernel_transformation(
    ...
    if is_query:
        data_dash = ratio * (
            torch.exp(
                data_dash - diag_data - torch.max(data_dash, dim=last_dims_t, keepdim=True)[0]
            )
...
class NodeFormer(nn.Module):
    ...
    def fit(self, graph, labels, train_mask, val_mask, test_mask):
        ...
        adjs = []
        adj, _ = remove_self_loops(torch.stack(graph.edges()).to(self.device))
        adj, _ = add_self_loops(adj, num_nodes=graph.num_nodes())
        adjs.append(adj)
        for i in range(self.args.rb_order - 1):  # edge_index of high order adjacency
            adj = adj_mul(adj, adj, graph.num_nodes())
            adjs.append(adj)
        self.adjs = adjs
        ...
        for epoch in range(self.epochs):
            ...
            Z, link_loss_ = self.forward(X, adjs, tau=self.args.tau)
            loss = loss_fn(Z[self.train_mask], labels[self.train_mask])
            loss -= self.args.lamda * sum(link_loss_) / len(link_loss_)
