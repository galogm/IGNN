...
class GBKGNN(nn.Module):
    ...
    def fit(self, graph, labels, train_mask, val_mask, test_mask):
        ...
        edge_train_mask = self.train_mask[edge_index[0]] * self.train_mask[edge_index[1]]
        ...
        for epoch in range(self.epochs):
            ...
            for sigma in sigma_list:
                ...
                regularizer = loss_function(
                    ...
                    torch.tensor(similarity, dtype=torch.long).to(self.device)[edge_train_mask],
                )
...
class MessagePassingNew(torch.nn.Module):
    ...
    def __init__(
        self, aggr: Optional[str] = "add", flow: str = "source_to_target", node_dim: int = -2
    ):
    ...
    def propagate(self, edge_index: Adj, size: Size = None, previous=None, **kwargs):
        ...
        # Otherwise, run both functions in separation.
            ...
            if previous is None:
                ...
                out, sigma = self.message_negative(msg_kwargs["x_j"], msg_kwargs["x_i"], previous)
...
class SAGEConvNew(MessagePassingNew):
    def __init__(
        ...
        **kwargs,
        ...
        if out_channels % 2 == 0:
            self.lin_p = Linear(in_channels[0], int(out_channels / 2), bias=bias)
            self.lin_n = Linear(in_channels[1], int(out_channels / 2), bias=bias)
        else:
    ...
    def forward(
        ...
        out, sigma = self.propagate(edge_index, x=x, sigma=(sigma_l, sigma_r), size=[None, None])
        out = self.lin_p(out)

        out_, _ = self.propagate(
            edge_index, x=x, sigma=(sigma_l, sigma_r), size=[None, None], previous=sigma
        )
