...
class MixHop(nn.Module):
    ...
    def fit(self, graph, labels, train_mask, val_mask, test_mask):
        ...
        X = graph.ndata["feat"]
        n_nodes, _ = X.shape
        adj = graph.adj_external(scipy_fmt="coo")
    ...
    def forward(self, features, normalized_adjacency_matrix, return_Z=False):
        ...
        abstract_features_1 = torch.cat(
            [
                self.upper_layers[i](normalized_adjacency_matrix, features)
        ...
        abstract_features_2 = torch.cat(
            [
                self.bottom_layers[i](normalized_adjacency_matrix, abstract_features_1)
    ...
    def setup_layer_structure(self, base_run):
        ...
        self.upper_layers = [
            SparseNGCNLayer(self.feature_num, self.layers_1[i - 1], i, self.dropout)
            for i in range(1, self.order_1 + 1)
        ]
        ...
        self.bottom_layers = [
            ...
            for i in range(1, self.order_2 + 1)
        ]
    ...
    def evaluate_architecture(self):
        ...
        self.layer_sizes["upper"] = [
            int(self.budget * layer_size / sum(self.layer_sizes["upper"]))
            for layer_size in self.layer_sizes["upper"]
        ]
        self.layer_sizes["bottom"] = [
            int(self.budget * layer_size / sum(self.layer_sizes["bottom"]))
            for layer_size in self.layer_sizes["bottom"]
        ]
...
class SparseNGCNLayer(torch.nn.Module):
    ...
    def forward(self, normalized_adjacency_matrix, features):
        ...
        base_features = torch.nn.functional.dropout(
            base_features, p=self.dropout_rate, training=self.training
        )
