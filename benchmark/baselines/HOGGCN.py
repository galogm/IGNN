...
class HOGGCN(nn.Module):
    def __init__(
        ...
        self.model_MLP = MLP_hog(
            n_feat=in_features, n_hid=args.nhid2, nclass=class_num, dropout=self.dropout
        )
...
def one_hot_embedding(labels, num_classes, soft, device):
...
class Attention(nn.Module):
    def __init__(self, in_size, hidden_size=32):
        ...
        self.project = nn.Sequential(
            nn.Linear(in_size, hidden_size), nn.Tanh(), nn.Linear(hidden_size, 1, bias=False)
        )
...
class GraphConvolution_homo(nn.Module):
    ...
    def reset_parameters(self):
        ...
        stdv_bi = 1.0 / math.sqrt(self.weight_bi.size(1))
    ...
    def forward(self, input, adj, bi_adj, output, labels_for_lp):
        ...
        with torch.no_grad():
            ...
            bi_r_inv[torch.isinf(bi_r_inv)] = 0.0
