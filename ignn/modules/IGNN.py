...
class IGNNConv(nn.Module):
            ...
            self.nei_ind_emb = nn.ModuleList(
                (
                    GraphConv(
                        ...
                        allow_zero_in_degree=self.RN == "attentive",
                        activation=acts[act](),
                    )
                    if i != 0
                    else MLP(
    ...
    def other_INs(self, features, graph, device):
        if self.IN in ["gcn-IN-nSN"]:
    ...
    def forward(
        ...
        if self.nei_feats is None:
            self.nei_feats, self.adj = self.inceptive_aggregation(
                ...
                fast=self.fast,
                return_adj=True,
                process_adj=self.adj is None,
                # fast c-IGNN on large graphs will aggregate on CPUs without batch load
                device=torch.device("cpu") if self.fast else device,
                batch_idx=None if self.fast else batch_idx,
            )
        ...
        if self.IN == "gcn-IN-SN":
            ...
            self.other_INs(features, graph, device)
